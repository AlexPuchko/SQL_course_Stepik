...
SELECT [DISTINCT | ALL ] /_ поля таблиц_/
FROM /_ из каких таблиц_/
WHERE /_ условие на ограничение строк_/
GROUP BY /_условие группировки_/
HAVING BY /_условие на ограничение строк после группировки_/
ORDER BY /_порядок сортировки_/ [ ASC | DESC]
LIMIT /_ ограничение колво записей_/
...
/_SELECT title AS Название, amount
FROM book;_/

IF(логическое*выражение, выражение_1, выражение_2)
Функция вычисляет логическое*выражение, если оно истина –
...
FROM book
GROUP BY author
HAVING SUM(price _ amount) > 5000; \*/
...
/_INSERT INTO book(title, author, price, amount)
VALUES ('Идиот', 'Достоевский Ф.М.', 460.00, 10);_/

DELETE - глагол "удали"
DELETE FROM таблица; - Этот запрос удаляет все записи из указанной после FROM таблицы.
/_DELETE FROM supply;_/
...
/_MONTH('2020-04-12') = 4
MONTH(date_first) месяц из столбца_/
...
/_ MONTHNAME('2020-04-12')='April' _/
...
7.  Колличество без NULL
    ...
    HAVING Количество < 10 /_HAVING IFNULL(SUM(book.amount), 0) < 10_/
    ORDER BY Количество;
...
select _ from tab1 where a + b > 10; --> search table tab1 using index ...
select _ from tab1 where b + a > 10; --> scan table
...
with tab2 as (select _ from tab1 where a > 10),
tab3 as (select _ from tab2 inner join ...)
select \* from tab3;
...
-- В main схеме
select _ from pragma_table_info('tab1');
-- В temp схеме или подключенной (attach) базе
select _ from pragma_table_info('tab1') where schema = 'temp'
